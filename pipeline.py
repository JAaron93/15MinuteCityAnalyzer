...
def __(block_groups, plt):
    ...
    block_groups.plot(
        ax=ax_bg, edgecolor="white", column="population", cmap="Blues", legend=True
    )
...
def __(amenities, plt):
    ...
    amenities.plot(
        ax=ax_am, column="amenity_type", legend=True, markersize=10, alpha=0.7
    )
...
def __(isochrones, plt):
    ...
    isochrones.plot(
        ax=ax_iso, alpha=0.3, edgecolor="k", column="amenity_type", legend=True
    )
...
def __(output_data, plt):
    ...
    output_data["accessibility_score"].hist(
        ax=ax_hist, bins=20, color="skyblue", edgecolor="k"
    )
