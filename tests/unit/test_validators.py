...
def test_validate_osm_data_basic():
    validator = DataValidator()
    df = pd.DataFrame(
        {"osm_id": ["1"], "amenity_type": ["grocery"], "geometry": [Point(0, 0)]}
    )
...
def test_validate_demographics():
    validator = DataValidator()
    df = pd.DataFrame({"population": [100, None], "median_income": [50000, None]})
    gdf = gpd.GeoDataFrame(df)
...
def test_validate_osm_data_null_geometry():
    ...
    validator = DataValidator()
    df = pd.DataFrame(
        {"amenity_type": ["grocery", "grocery"], "geometry": [Point(0, 0), None]}
    )
    gdf = gpd.GeoDataFrame(df, crs="EPSG:4326")
