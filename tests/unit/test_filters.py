...
from src.dashboard.filters import MAX_INCOME_THRESHOLD, apply_all_filters
...
def test_apply_all_filters_income(sample_gdf):
    ...
    filtered = apply_all_filters(
        sample_gdf, income_threshold=50000, score_range=(0, 100)
    )
...
def test_apply_all_filters_score(sample_gdf):
    ...
    filtered = apply_all_filters(
        sample_gdf, income_threshold=MAX_INCOME_THRESHOLD, score_range=(40, 90)
    )
...
def test_apply_all_filters_income_out_of_range(sample_gdf):
    ...
    with pytest.raises(
        ValueError, match=r"income_threshold must be between 0.0 and 200000.0"
    ):
    ...
    with pytest.raises(
        ...
        apply_all_filters(
            sample_gdf, income_threshold=MAX_INCOME_THRESHOLD + 1, score_range=(0, 100)
        )
...
def test_apply_all_filters_score_range_invalid_order(sample_gdf):
    ...
    with pytest.raises(
        ValueError, match="score_range min_score .* must be <= max_score"
    ):
...
def test_apply_all_filters_invalid_types(sample_gdf):
    ...
    with pytest.raises(
        ValueError, match="score_range must be a tuple or list of length 2."
    ):
...
def test_apply_all_filters_empty_gdf():
    df = pd.DataFrame(
        columns=["geoid", "median_income", "accessibility_score", "geometry"]
    )
...
def test_apply_all_filters_boundaries(sample_gdf):
    ...
    filtered = apply_all_filters(
        sample_gdf, income_threshold=30000, score_range=(20, 20)
    )
    assert len(filtered) == 1
    assert filtered.iloc[0]["geoid"] == "1"
...
def test_apply_all_filters_none_values():
    ...
    filtered = apply_all_filters(
        gdf, income_threshold=MAX_INCOME_THRESHOLD, score_range=(0, 100)
    )
    assert len(filtered) == 1
    assert filtered.iloc[0]["geoid"] == "1"
