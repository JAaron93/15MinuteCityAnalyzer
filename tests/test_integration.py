...
from src.pipeline.crs_utils import WGS84
...
def generate_mock_data():
    ...
    block_groups = gpd.GeoDataFrame(
        {"geoid": ["060650400001"], "population": [1500], "median_income": [65000.0]},
        geometry=[bg_geom],
        crs=WGS84,
    )
    ...
    isochrones = gpd.GeoDataFrame(
        {"amenity_type": ["grocery"]}, geometry=[iso_geom], crs=WGS84
    )
...
def test_pipeline_to_dashboard_integration():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # With only 1 block group, score is normalized to 50.0 (city_min == city_max case)  # noqa: E501
