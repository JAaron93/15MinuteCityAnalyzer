...
get_repo_root() {
    ...
    local script_dir
    script_dir="$(CDPATH="" cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)" || true
    if [ -z "$script_dir" ]; then
        echo "get_repo_root: could not resolve script directory from BASH_SOURCE (${BASH_SOURCE[0]:-<unset>})" >&2
        return 1
    fi
    local fallback_root
    if ! fallback_root="$(cd "$script_dir/../../.." && pwd)"; then
        echo "get_repo_root: fallback cd failed — script_dir='$script_dir' does not have three ancestor directories" >&2
        return 1
    fi
    echo "$fallback_root"
}
