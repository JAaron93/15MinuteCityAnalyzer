...
$maxBranchLength = 244
$branchNameByteCount = [System.Text.Encoding]::UTF8.GetByteCount($branchName)
if ($branchNameByteCount -gt $maxBranchLength) {
    $prefixLength = $featureNum.Length + 1
    # For truncation, we need to work with character positions
    # but ensure the result fits in 244 bytes
    $prefixByteLength = [System.Text.Encoding]::UTF8.GetByteCount($featureNum) + 1
    $maxSuffixBytes = $maxBranchLength - $prefixByteLength

    # Truncate suffix by characters, then verify byte count
    $truncatedSuffix = $branchSuffix
    while ([System.Text.Encoding]::UTF8.GetByteCount($truncatedSuffix) -gt $maxSuffixBytes -and $truncatedSuffix.Length -gt 0) {
...
, ''
    ...
    Write-Warning "[specify] Original: $originalBranchName ($branchNameByteCount bytes)"
