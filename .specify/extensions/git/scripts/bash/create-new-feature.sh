...
check_existing_branches() {
    ...
    if [ "$skip_fetch" = true ]; then
        ...
        # Pruning is omitted by default to avoid surprising deletions of remote-tracking refs.
...
if [ -n "${GIT_BRANCH_NAME:-}" ] && [ "$BRANCH_BYTE_LEN" -gt $MAX_BRANCH_LENGTH ]; then
    ...
    TRUNCATED_SUFFIX=$(echo "$BRANCH_SUFFIX" | cut -b1-$MAX_SUFFIX_LENGTH)
...
if $JSON_MODE; then
    if command -v jq >/dev/null 2>&1; then
        ...
        if type json_escape >/dev/null 2>&1; then
            ...
            _je_branch=$(_basic_json_escape "$BRANCH_NAME")
            _je_num=$(_basic_json_escape "$FEATURE_NUM")
        fi
        if [ "$DRY_RUN" = true ]; then
