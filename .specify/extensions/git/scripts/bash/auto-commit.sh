...
if [ -f "$_config_file" ]; then
    ...
    # If event-specific enabled key not found, use default
    if [ "$_explicit_event_enabled" = "false" ] && [ "$_default_enabled" = "true" ]; then
        _enabled=true
    fi
else
