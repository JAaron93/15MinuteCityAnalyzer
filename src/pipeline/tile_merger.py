...
class TileMerger:
    ...
    def merge_pois(self, tiles_data: List[gpd.GeoDataFrame]) -> gpd.GeoDataFrame:
        ...
        if not non_points.empty:
            ...
            group_cols = [
                c for c in ["element_type", "osmid"] if c in non_points_flat.columns
            ]
            if not group_cols:
            ...
            if group_cols and non_points_flat.duplicated(subset=group_cols).any():
                logger.info(
                    f"Unioning split polygons/linestrings using grouping: {group_cols}"
                )

                def _union_geoms(group: pd.DataFrame) -> pd.DataFrame:
                ...
                non_points = non_points_flat.groupby(
                    group_cols, group_keys=False
                ).apply(_union_geoms)

                # Restore the original index structure if possible
                restorable_names = [
                    name for name in original_index_names if name is not None
                ]
                if restorable_names and all(
                    name in non_points.columns for name in restorable_names
                ):
    ...
    def merge_graphs(self, tiles_data: List[Any]) -> Any:
        ...
        duplicates = nodes_df.groupby(["x_round", "y_round"]).filter(
            lambda x: len(x) > 1
        )

        if not duplicates.empty:
            logger.info(
                f"Rejoining {len(duplicates)} spatially identical nodes across tile boundaries."
            )
            node_mapping = {}
            ...
            if self_loops_to_remove:
                logger.info(
                    f"Removing {len(self_loops_to_remove)} newly created self-loops."
                )
